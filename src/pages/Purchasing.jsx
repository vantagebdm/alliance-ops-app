...
export default function Purchasing() {
  ...
  useEffect(() => {
    base44.entities.PurchaseOrder.list("-created_date", 100).then(d => {
      setOrders(d);
      setLoading(false);
    });
  }, []);
  ...
  const columns = [
    { key: "po_number", label: "PO #", render: (v) => <span className="font-mono font-semibold">{v || "—"}</span> },
    { key: "supplier_name", label: "Supplier" },
    { key: "status", label: "Status", render: (v) => <StatusBadge status={v} /> },
    { key: "total", label: "Total", render: (v) => `$${(v || 0).toLocaleString("en-AU", { minimumFractionDigits: 2 })}` },
    { key: "expected_date", label: "Expected", render: (v) => v ? moment(v).format("DD/MM/YY") : "—" },
    ...
    <div>
      ...
      <div className="p-6 space-y-4">
        ...
        {loading ? (
          ...
          <DataTable columns={columns} data={filtered} onRowClick={(row) => navigate(`/purchasing/${row.id}`)} emptyMessage="No purchase orders found." />
        )}
      </div>
    </div>
  );
