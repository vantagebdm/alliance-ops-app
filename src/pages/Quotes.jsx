...
export default function Quotes() {
  ...
  useEffect(() => {
    base44.entities.Quote.list("-created_date", 100).then(d => {
      setQuotes(d);
      setLoading(false);
    });
  }, []);
  ...
  const columns = [
    { key: "quote_number", label: "Quote #", render: (v) => <span className="font-mono font-semibold">{v || "—"}</span> },
    { key: "customer_name", label: "Customer" },
    { key: "company", label: "Company" },
    { key: "status", label: "Status", render: (v) => <StatusBadge status={v} /> },
    { key: "total", label: "Total", render: (v) => `$${(v || 0).toLocaleString("en-AU", { minimumFractionDigits: 2 })}` },
    { key: "valid_until", label: "Valid Until", render: (v) => v ? moment(v).format("DD/MM/YY") : "—" },
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Button onClick={() => navigate("/quotes/new")} className="bg-primary text-black font-heading font-semibold uppercase text-xs tracking-wider hover:bg-primary/90 rounded-sm">
      ...
      <div className="p-6 space-y-4">
        ...
        {loading ? (
          ...
          <DataTable columns={columns} data={filtered} onRowClick={(row) => navigate(`/quotes/${row.id}`)} emptyMessage="No quotes found." />
        )}
      </div>
    </div>
  );
