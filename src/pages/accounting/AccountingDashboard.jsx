...
const KPICard = ({ label, value, sub, icon: Icon, color = "text-foreground", alert }) => (
  <div className={`bg-card border border-border rounded-sm p-4 flex flex-col gap-2 ${alert ? "border-red-500/40" : ""}`}>
    <div className="flex items-center justify-between">
      <span className="font-heading text-[9px] uppercase tracking-widest text-muted-foreground">{label}</span>
...
export default function AccountingDashboard() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h2 className="font-heading text-base font-bold text-foreground uppercase tracking-wider mb-1">Accounting Dashboard</h2>
        <p className="text-xs text-muted-foreground">Financial overview — {now.toLocaleDateString("en-AU", { month: "long", year: "numeric" })}</p>
      </div>

      {/* Bank & Cashflow */}
      <div>
        <div className="font-heading text-[9px] uppercase tracking-widest text-muted-foreground mb-2">Bank & Cashflow</div>
      ...
      {/* Monthly P&L */}
      <div>
        <div className="font-heading text-[9px] uppercase tracking-widest text-muted-foreground mb-2">This Month — P&L Snapshot</div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      {/* GST / BAS */}
      <div>
        <div className="font-heading text-[9px] uppercase tracking-widest text-muted-foreground mb-2">GST & BAS Estimate (This Month)</div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              {overdueInv.slice(0, 5).map(i => (
                <div key={i.id} className="flex justify-between text-xs">
                  <span className="text-foreground">{i.customer_name}</span>
        ...
        <div className="bg-card border border-amber-500/30 rounded-sm p-4">
          ...
          {bills.filter(b=>["approved","overdue"].includes(b.status)).length === 0 ? (
            <div className="text-xs text-muted-foreground">No bills outstanding</div>
          ) : (
            <div className="space-y-1.5">
              {bills.filter(b=>["approved","overdue"].includes(b.status)).slice(0, 5).map(b => (
                <div key={b.id} className="flex justify-between text-xs">
                  <span className={`${b.status === "overdue" ? "text-red-400" : "text-foreground"}`}>{b.supplier_name}</span>
      ...
      <div className="bg-card border border-border rounded-sm p-4">
        <div className="font-heading text-[9px] uppercase tracking-widest text-muted-foreground mb-3">Bank Accounts</div>
        {bankAccounts.length === 0 ? (
          <div className="text-xs text-muted-foreground">No bank accounts configured. Go to Bank & Reconciliation to add accounts.</div>
        ) : (
          <div className="space-y-2">
            {bankAccounts.map(a => (
              <div key={a.id} className="flex justify-between items-center text-sm border-b border-border pb-2 last:border-0">
                <div>
                  <span className="text-foreground font-medium">{a.name}</span>
                  <span className="text-muted-foreground text-xs ml-2">{a.bank_name}</span>
                </div>
