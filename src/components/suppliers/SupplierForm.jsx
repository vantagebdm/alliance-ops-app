...
export default function SupplierForm({ onClose, onSaved, initial }) {
  ...
  const save = async () => {
    setSaving(true);
    if (initial?.id) await base44.entities.Supplier.update(initial.id, form);
      ...
      <div className="bg-white w-full max-w-lg rounded-sm shadow-2xl mb-10">
        ...
        <div className="p-6 grid grid-cols-2 gap-3">
          <div className="col-span-2">
            <label className="font-heading text-[11px] uppercase tracking-wider text-foreground/60 mb-1 block">Company Name *</label>
          ...
          <div>
            ...
            <Input value={form.contact_person} onChange={e => update("contact_person", e.target.value)} className="rounded-sm" />
          </div>
          <div>
        ...
        <div className="px-6 py-4 bg-muted/30 border-t border-border flex justify-end gap-3">
          ...
          <Button onClick={save} disabled={saving || !form.name}
            ...
            {saving ? "Saving..." : initial ? "Update" : "Add Supplier"}
          </Button>
        </div>
      </div>
    </div>
