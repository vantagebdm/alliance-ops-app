        ...
        <div className="w-80 flex-shrink-0 border-r border-border bg-card p-5 flex flex-col gap-4 overflow-y-auto">
            ...
            <div className="space-y-3">
              <div>
                ...
                <Select value={form.status} onValueChange={v => u("status", v)}>
                  ...
                  <SelectContent>
                    {["pending","confirmed","processing","ready","dispatched","delivered","cancelled"].map(s => (
              ...
              <div>
                <label className="font-heading text-[10px] uppercase tracking-wider text-foreground/50 mb-1 block">Priority</label>
              ...
              <div>
                <label className="font-heading text-[10px] uppercase tracking-wider text-foreground/50 mb-1 block">Delivery Method</label>
              ...
              {form.delivery_method !== "pickup" && (
                <div>
                  <label className="font-heading text-[10px] uppercase tracking-wider text-foreground/50 mb-1 block">Delivery Address</label>
          ...
          <div className="flex-1">
            ...
            <Textarea value={form.notes} onChange={e => u("notes", e.target.value)}
              placeholder="Internal notes, special handling..." className="rounded-sm h-32 resize-none" />
          </div>
        </div>
                    ...
                    <td className="px-2 py-1.5">
                      <PartAutocomplete
                        ...
                        onClear={() => {
                          const items = form.items.map((line, idx) => {
                            if (idx !== i) return line;
                            return { ...line, part_number: "", description: "", unit_price: 0, total: 0 };
                          });
                          const subtotal = items.reduce((s, l) => s + (Number(l.total) || 0), 0);
                          setForm(f => ({ ...f, items, subtotal, gst: subtotal * 0.1, total: subtotal * 1.1 }));
                        }}
